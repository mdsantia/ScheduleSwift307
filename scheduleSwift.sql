...
CREATE TABLE `facilityData` (
  ...
  `numPeople` VARCHAR(200) DEFAULT NULL,
  `numReservable` VARCHAR(200) DEFAULT NULL,
  `paymentRequire` VARCHAR(200) DEFAULT NULL,
  `paymentValue` VARCHAR(45) DEFAULT NULL,
...
CREATE TABLE `shifts` (
  ...
  `timeClockedIn` VARCHAR(200) NULL,
  `timeClockedOut` VARCHAR(200) NULL,
  `timeOfShift` VARCHAR(200) NULL,
...
CREATE TABLE `managerContacts` (
  ...
  `actualContact` VARCHAR(200) NULL,
  PRIMARY KEY (`ID`)
) ENGINE=INNODB AUTO_INCREMENT=57 DEFAULT CHARSET=UTF8MB4 COLLATE = UTF8MB4_0900_AI_CI;

CREATE TABLE `dateExceptions` (
  `ID` INT NOT NULL AUTO_INCREMENT,
  `businessName` VARCHAR(200) NULL,
  `date` VARCHAR(200) NULL,
