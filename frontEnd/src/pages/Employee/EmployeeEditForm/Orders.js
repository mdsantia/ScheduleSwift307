                ...
                <Box component="form" validate="true" onSubmit={handleSubmit} sx={{ mt: 3 }}>
                    ...
                    <Button
                        ...
                        disabled={ (validForm() && !(new Date(currentDate).getTime() === new Date(storedCurrentDate).getTime() && new Date(startTime).getTime() === new Date(storedStartTime).getTime() &&
                            new Date(endTime).getTime() === new Date(storedEndTime).getTime() && parseInt(numPeople) === parseInt(storedNumPeople) && (_.isEqual(numArray, storedNumArray)))) ? false : true || stopEdit === "Yes"}
                        fullWidth
                        variant="contained"
                        sx={{ mt: 3, mb: 2 }}
                    >
